...
class MyApp(ShowBase):
    ...
    def __change_class(self, incr):
        self.curr_ease[0] = easeClass[
            (easeClass.index(self.curr_ease[0]) + incr) % len(easeClass)]
        self.class_lab.setText(self.curr_ease[0])

    def __change_type(self, incr):
        self.curr_ease[1] = easeType[
            (easeType.index(self.curr_ease[1]) + incr) % len(easeType)]
